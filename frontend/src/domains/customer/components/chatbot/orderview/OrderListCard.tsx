...
import {
  formatCurrencyVnd,
  formatShortDate,
} from '@/domains/customer/helpers/formatters'
import { Button } from '@/shared/components/ui/button'

interface OrderListCardProps {
  order: OrderListItem
  onSubmitTracking: (data: string) => void
}
...
export const OrderListCard = ({
  ...
  const handleTracking = (orderId: string) => {
    onSubmitTracking(orderId)
  }

  return (
    <Card className='border-border/60 bg-card/80 shadow-sm hover:shadow-md transition-all'>
      <CardContent className='p-4 space-y-3'>
        ...
        <div className='flex items-center gap-1 justify-start'>
          <Button onClick={() => handleTracking(order.id)}>
            <Truck />
            Tracking Order
          </Button>
        </div>

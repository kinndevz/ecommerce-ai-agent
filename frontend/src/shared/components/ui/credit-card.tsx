...
export const CreditCardFlipper = ({
  ...
  return (
    <CreditCardFlipContext.Provider value={true}>
      <div
        className={cn(
          'h-full w-full rounded-2xl transform-3d transition duration-700 ease-in-out',
          supportsHover &&
...
export const CreditCardChip = ({
  ...
  <svg
    className={cn(
      'absolute top-1/2 -translate-y-1/2 left-0 w-1/6 shrink-0 rounded-[18%]',
      className
    )}
...
export const CreditCardFront = ({
  ...
  <div
    className={cn(
      'backface-hidden absolute inset-0 flex overflow-hidden rounded-2xl',
      className
    )}
    {...props}
  >
...
const CreditCardBackContext = createContext<{ safeArea: number }>({
  safeArea: 20,
})
